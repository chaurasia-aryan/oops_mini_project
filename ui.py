# ui.py
# TakeBook (Facebook clone) with DB + Khelo Kudo section
...
from cv import play_hand_snake, VideoFilterWindow
...
class FacebookHome:
    ...
    def show_khelo(self):
        ...
        tk.Button(self.feed_frame, text="Play Snake 🐍", bg=PRIMARY_COLOR, fg="white", font=(FONT_BOLD, 12),
                  command=lambda: threading.Thread(target=play_hand_snake, daemon=True).start()).pack(pady=15, ipadx=10, ipady=5)
        tk.Button(self.feed_frame, text="Live Filters 🎥", bg="#2ecc71", fg="white", font=(FONT_BOLD, 12),
                  command=lambda: VideoFilterWindow(self.root)).pack(pady=15, ipadx=10, ipady=5)
